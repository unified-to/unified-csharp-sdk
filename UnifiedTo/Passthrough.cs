    ...
    public class Passthrough: IPassthrough
    {
        ...
        public async Task<CreatePassthroughResponse> CreatePassthroughAsync(string connectionId, string path, Dictionary<string, object>? requestBody = null)
        {
            ...
            if((response.StatusCode == 200))
            {
                ...
                {
                    response.Result = JsonConvert.DeserializeObject<Dictionary<string, object>>(await httpResponse.Content.ReadAsStringAsync(), new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = new JsonConverter[] { new FlexibleObjectDeserializer(), new EnumConverter(), new AnyDeserializer() }});
                }
