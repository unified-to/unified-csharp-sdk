    ...
    public class Storage: IStorage
    {
        ...
        public async Task<CreateStorageFileResponse> CreateStorageFileAsync(string connectionId, StorageFile? storageFile = null)
        {
            ...
            if((response.StatusCode == 200))
            {
                ...
                {
                    response.StorageFile = JsonConvert.DeserializeObject<StorageFile>(await httpResponse.Content.ReadAsStringAsync(), new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = new JsonConverter[] { new FlexibleObjectDeserializer(), new EnumConverter(), new AnyDeserializer() }});
                }
        ...
        public async Task<ListStorageFilesResponse> ListStorageFilesAsync(ListStorageFilesRequest request)
        {
            ...
            if((response.StatusCode == 200))
            {
                if(Utilities.IsContentTypeMatch("application/json",response.ContentType))
                {
                    response.StorageFiles = JsonConvert.DeserializeObject<List<StorageFile>>(await httpResponse.Content.ReadAsStringAsync(), new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = new JsonConverter[] { new FlexibleObjectDeserializer(), new EnumConverter(), new AnyDeserializer() }});
                }
